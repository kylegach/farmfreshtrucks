...
gulp.task('rev-font-workaround', ['rev-assets'], function() {
  var manifest = require('../../.' + config.buildAssets + '/rev-manifest.json');
  var fontList = [];

  _.each(manifest, function(reference, key) {
    var fontPath = config.iconFont.dest.split(config.buildAssets)[1].substr(1);
  ...
  var streams = fontList.map(function(file) {
    ...
    return gulp.src(config.buildAssets + '/' + file.path + '*.!(svg)')
      .pipe(rename({suffix: file.hash}))
      .pipe(gulp.dest(config.iconFont.dest));
  });

  // Re-write rev-manifest.json to disk
  fs.writeFile(config.buildAssets + '/rev-manifest.json', JSON.stringify(manifest, null, 2));
